...
import LoginForm from "../../authPage/LoginPage"; // 👈 import your login form
...
const Header = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full bg-white shadow-sm z-50">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <ul className="hidden lg:flex items-center space-x-8 text-gray-700 font-medium gap-4">
            <li
              className="hover:text-purple-600 cursor-pointer font-bold"
              onClick={() => navigate("/")}
            >
              Home
            </li>
            ...
            <li className="relative group cursor-pointer">
              <div className="flex items-center space-x-1 hover:text-purple-600 font-bold ">
                <span>Events</span>
                <FiChevronDown />
              </div>
              <ul className="absolute left-0 hidden group-hover:block bg-white shadow-md mt-2 rounded-md w-44">
                <li
                  className="px-4 py-2 hover:bg-purple-100 font-bold"
          ...
          <div className="hidden lg:flex items-center space-x-4 gap-5">
            <button
              ...
              className="border text-white px-4 bg-purple-600 py-1 rounded-md hover:bg-purple-50 hover:text-purple-600 transition"
            >
              Login
            </button>
            <a
        ...
        >
          <ul className="flex flex-col space-y-4 p-6 text-gray-700 font-medium text-left">
            ...
            <button
              onClick={() => setShowLogin(true)} // 👈 show login modal
              className="border border-purple-500 text-purple-600 px-4 py-2 rounded-md hover:bg-purple-50 transition"
            >
              Login
            </button>
            <a
      ...
      {/* Login Modal (Shared for Desktop & Mobile) */}
      {showLogin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative bg-white rounded-2xl shadow-lg w-full max-w-md">
            {/* Close button */}
            <button
            ...
            <LoginForm
              onSubmit={(data) => {
                ...
                setShowLogin(false); // close on success
              }}
            />
          </div>
        </div>
